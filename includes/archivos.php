...
class  Archivo {
  ...
  public $userPath = SITE_ROOT.DS.'..'.DS.'uploads/file';
...
/*--------------------------------------------------------------*/
 private function update_userImg($id){
  ...
  $sql .=" archivo='{$db->escape($this->fileName)}'";
...
/*--------------------------------------------------------------*/
  public function media_destroy($id,$file_name){
    ...
    if(empty($this->fileName)){
        $this->errors[] = "Falta el archivo.";
        return false;
      }
    if(!$id){
      $this->errors[] = "ID del archivo ausente.";
      return false;
    }
    if(delete_by_id('media',$id)){
      ...
      $this->error[] = "Se ha producido un error en la eliminación del archivo.";
      return false;
    }
