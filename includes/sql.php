...
function delete_by_protocolo($table, $id, $activo)
{
  global $db;
  if (tableExists($table)) {
    $sql = "UPDATE " . $db->escape($table);
...
{
  global $db;
  $sql  = " SELECT a.matricula AS matricula,a.nombre AS nombre,a.primerAp AS primerAp,a.SegundoAp AS SegundoAp,a.fecha_nacimiento AS fecha,a.email AS email,a.activo AS activo,a.Semestre AS Semestre,l.nombre AS Licenciatura";
  $sql  .= " FROM alumnos a, licenciatura l";
  $sql .= " WHERE a.Licenciatura = l.claveLic ";
  $sql  .= " ORDER BY matricula ASC";
  return find_by_sql($sql);
}function join_asesorados_table()
{
  global $db;
...
{
  global $db;
  $sql  = " SELECT a.matricula AS matricula,a.nombre AS nombre,a.primerAp AS primerAp,a.SegundoAp AS SegundoAp,a.fecha_nacimiento AS edad,a.email AS email,a.activo AS activo,a.Semestre AS Semestre,l.nombre AS Licenciatura";
  $sql  .= " FROM alumnos a, licenciatura l, tutores t";
  $sql .= " WHERE a.Licenciatura = l.claveLic AND a.activo='Activo' AND t.claveD = '{$idD}' AND a.matricula = t.matricula  ";
  $sql  .= " ORDER BY matricula ASC";
  return find_by_sql($sql);
}
function join_asesor2_table_alumnos($idD)
{
  global $db;
  $sql  = " SELECT t.claveD AS claveD, m.nombre AS nombre";
  $sql  .= " FROM tutores t, maestros m";
  $sql .= " WHERE t.matricula = '{$idD}' AND m.claveD = t.claveD  ";
  $sql  .= " ORDER BY matricula ASC";
  return find_by_sql($sql);
}
function join_protocolo_table($docente)
{
  global $db;
  $sql  = " SELECT i.id AS id, m.nombre AS modalidad, i.matricula AS matricula, i.nombre AS Proyecto, i.tematica AS Tematica, i.estado AS estado, a.nombre AS nombre";
  $sql .= " FROM modalidad m, inducion i, alumnos a";
  $sql .= " WHERE a.matricula = i.matricula AND i.modalidad = m.id AND i.claveD = '{$docente}'";
  $sql .= " ORDER BY m.id ASC";
  return find_by_sql($sql);
}
function join_inducion_table($docente)
{
  global $db;
  $sql  = " SELECT i.id AS id,i.nombre AS proyecto,i.matricula AS matricula,i.estado AS estado,i.evaluacion AS evaluacion, a.nombre AS nombre, i.modalidad AS modalidad";
  $sql .= " FROM inducion i, alumnos a";
  $sql .= " WHERE a.matricula = i.matricula AND i.claveD = '{$docente}'";
...
function join_sesiones_table($docente)
{
  global $db;
  ...
  $sql  .=" WHERE s.matricula = a.matricula AND s.claveD = '{$docente}'";
...
function join_Act_table($docente)
{
  global $db;
  $sql  = " SELECT s.archivo, s.actividad AS actividad, s.fecha AS fecha, a.matricula AS matricula, a.nombre AS nombre, s.id AS id, s.estado As estado";
  $sql  .= " FROM actividad s, alumnos a";
  $sql  .=" WHERE s.matricula = a.matricula AND s.claveD = '{$docente}'";
  $sql .= " ORDER BY s.id ASC";
  return find_by_sql($sql);
}
function join_diagnostico_table($docente)
{
  global $db;
  ...
  $sql  .=" WHERE d.matricula = a.matricula AND d.claveD = '{$docente}'";
...
{
  global $db;
  $sql  = " SELECT i.id AS id, i.nombre AS nombre, i.tematica AS tematica, i.evaluacion AS evaluacion, m.nombre AS modalidad";
  $sql  .= " FROM modalidad m, inducion i";
  $sql  .=" WHERE matricula = $matricula AND estado = 'Activo' AND i.modalidad = m.id";
  $sql  .= " ORDER BY id ASC";
  return find_by_sql($sql);
}
...
function join_habilidad_table($docente)
{
  global $db;
  $sql  = " SELECT h.id AS id,  h.matricula AS matricula, h.fecha AS fecha, h.observacion AS observacion, h.evaluacion AS evaluacion, 
  h.evidencia AS evidencia, h.aspectos AS aspectos, h.fortaleza AS fortaleza, h.formas AS formas, 
  h.preguntas AS preguntas, h.reflexion AS refelxion, a.nombre AS nombre, h.atencion AS atencion";
  $sql  .= " FROM habilidad h, alumnos a";
  $sql  .=" WHERE h.claveD = '$docente' AND h.matricula = a.matricula";
  $sql  .= " ORDER BY id ASC";
  return find_by_sql($sql);
}
function join_habilidad_table_alumno($alumno)
{
  global $db;
  $sql  = " SELECT h.id AS id,  h.matricula AS matricula, h.fecha AS fecha, h.observacion AS observacion, h.evaluacion AS evaluacion, 
  h.evidencia AS evidencia, h.aspectos AS aspectos, h.fortaleza AS fortaleza, h.formas AS formas, 
  h.preguntas AS preguntas, h.reflexion AS refelxion, a.nombre AS nombre, h.atencion AS atencion";
  $sql  .= " FROM habilidad h, alumnos a";
  $sql  .=" WHERE h.matricula = '$alumno' AND h.matricula = a.matricula";
  $sql  .= " ORDER BY id ASC";
  return find_by_sql($sql);
}
function join_biblioteca_ase_tit_alumno($alumno)
{
  global $db;
  global $db;
  $sql  = " SELECT nombre, id";
  $sql  .= " FROM biblioteca_ase_tit";
  $sql  .=" WHERE matricula = '$alumno'";
  $sql  .= " ORDER BY id ASC";
  return find_by_sql($sql);
}
function join_biblioteca_tut_pro_alumno($alumno)
{
  global $db;
  $sql  = " SELECT nombre, id";
  $sql  .= " FROM biblioteca_tut_pro";
  $sql  .=" WHERE matricula = '$alumno'";
  $sql  .= " ORDER BY id ASC";
  return find_by_sql($sql);
}
function join_biblioteca_ase_tit($docente)
{
  global $db;
  global $db;
  $sql  = " SELECT b.nombre, b.id, a.matricula AS matricula, a.nombre AS alumno";
  $sql  .= " FROM biblioteca_ase_tit b, alumnos a";
  $sql  .=" WHERE b.claveD = '$docente' AND b.matricula = a.matricula";
  $sql  .= " ORDER BY id ASC";
  return find_by_sql($sql);
}
function join_biblioteca_tut_pro($docente)
{
  global $db;
  $sql  = " SELECT b.nombre, b.id, a.matricula AS matricula, a.nombre AS alumno";
  $sql  .= " FROM biblioteca_tut_pro b, alumnos a";
  $sql  .=" WHERE b.claveD = '$docente' AND b.matricula = a.matricula";
  $sql  .= " ORDER BY id ASC";
  return find_by_sql($sql);
}
